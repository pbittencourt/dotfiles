...
i)
  ...
  append_new_term
  mantoread=$(echo "$input" | cut -c 2- | xargs)
  exec brave-browser "https://www.imdb.com/find/?q=$mantoread" &> /dev/null &
  ;;
o)
  # Search on opensubtitles
  append_new_term
  mantoread=$(echo "$input" | cut -c 2- | xargs)
  exec brave-browser "https://www.opensubtitles.org/pt/search/sublanguageid-por,pob/moviename-$mantoread" &> /dev/null &
  ;;
t)
  # Search on tldr
  append_new_term
